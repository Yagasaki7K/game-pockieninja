...
function OnUse(jssystem, self, user)
{
    ...
    var itembase = jssystem.GetItemBaseSystem();
...
//            return;
//        }
//        if (kernel.IsClass(self,ClassIndex.CLASS_SUSTAINITEM))
//        {
//            if (kernel.QueryInt(self, PropertyIndex.ItemType2) == 1)
//            {
//                itembase.IsInUseCDTime(user, kernel.QueryInt(self, PropertyIndex.UseCDTimeType), 1);
//            }
//        }
        ...
        //保存config
        var itemConfigs = dropsystem.GetItemConfigs(dropitemlist);
        ...
        if (kernel.QueryInt(self, PropertyIndex.ItemIsAppendLog) > 0)
        {
            kernel.SavePlayerLog(user, LogDefine.LOG_TYPE_USEITEM, selfConfig, String(kernel.QueryInt(user, PropertyIndex.Level)), "", "", kernel.GetUID(self), "");
        }
        ...
        for (var i = 0; i < dropitemlist.size(); i++)
        {
            ...
            var flag = false;
            if (!kernel.Exists(dItem))
            {
                //物品由于叠加而导致对象丢失时
                flag = true;
                var oldConfig = itemConfigs.get(dItem);
                if (oldConfig == null)
                {
                    continue;
                }
                dItem = kernel.Create(oldConfig);
                if (!kernel.Exists(dItem))
                {
                    continue;
                }
            ...
            var s = new VarList();
            s.AddString(kernel.QueryString(dItem, PropertyIndex.ItemName));
            ...
            else if (selfConfig == "i152043")
            {
                //神秘罐子
                if (kernel.QueryInt(dItem, PropertyIndex.ItemIsBroadcast) == 1)
                {
                    lookSystem.ShowItems(user, 122, dItem, null, false);
                }
            }
            ...
            else if (selfConfig == "i159042")
            {
                ...
                {
                    lookSystem.ShowItems(user, 128, dItem, null, false);
                }
            }
    ...
    {
        kernel.SavePlayerLog(user, LogDefine.LOG_TYPE_USEITEM, selfConfig, String(kernel.QueryInt(user, PropertyIndex.Level)), String(droppropertyvalue), "", "", "");
    ...
    if (kernel.IsClass(self, ClassIndex.CLASS_SUSTAINITEM))
    {
        kernel.IncInt(self, PropertyIndex.ItemFoldNum, 0);
    }
    else
    {
